...
@vite('resources/css/app.css')
    @vite('resources/css/admin/notifications.css')
        ...
        <div id="notif-container" style="position: fixed; top: 20px; right: 20px; z-index: 9999;">
        </div>
            ...
            <div class="max-w-7xl mx-auto">
                <div class="notifications-header">
                    ...
                    <div class="notifications-actions">
                        ...
                        <button id="bulkDeleteBtn" type="button" class="btn btn-danger" disabled>Delete
                            Selected</button>
                        ...
                        @foreach($notifications as $notification)
                            <div class="notification-card {{ $notification->is_read ? 'read' : 'unread' }}"
                                ...
                                @php
                                    ...
                                    ->first();
                                    $hasAdminReply = $notification->comment->replies->contains(fn ($reply) => !is_null($reply->admin_id));
                                @endphp

                                <div class="notification-content">
                                    <div class="notification-student-info">
                                        <strong>
                                            {{ $notification->comment->student->first_name ?? '' }}
                                            {{ $notification->comment->student->last_name ?? '' }}
                                        </strong>
                                    </div>
                                    ...
                                    @if($latestReply)
                                        <div class="notification-existing-reply">
                                            <div class="notification-reply-author">
                                                {{ $latestReply->admin?->first_name }}
                                                {{ $latestReply->admin?->last_name }}
                                            </div>
                                            ...
                                            <div class="notification-reply-date">
                                                {{ $latestReply->created_at->format('M d, Y H:i') }}
                                            </div>
                                        </div>
                                    @endif
                                    @if(!$hasAdminReply)
                                    <div class="notification-reply-section">
                                        ...
                                        <div id="replyBox-{{ $notification->id }}"
                                            class="notification-reply-box hidden">
